...
class PaymentDBImpl extends PaymentDBAlg[ConnectionIO] {
  ...
  def initDB: ConnectionIO[Unit] = for {
    _ <- createPaymentsTable()
    _ <- createTransactionsTable()
    _ <- createDiscountsTable()
  } yield ()

  def createPaymentsTable(): ConnectionIO[Int] =
    sql"""
  ...
  def createTransactionsTable(): ConnectionIO[Int] =
    sql"""
  ...
  def createDiscountsTable(): ConnectionIO[Int] =
    sql"""
